...
export async function GET({ props }: { props: SlugProps }) {
  ...
  const body = [
    ...
    ...(socialImage
      ? [`image: ${JSON.stringify(new URL(withBasePath(socialImage), new URL(config.site).origin).href)}`]
      : []),
    ...(version ? [`version: ${JSON.stringify(version)}`] : []),
    "---",
    "",
    "> Documentation Index",
    `> Fetch the complete documentation index at: ${new URL(withBasePath("/llms.txt"), new URL(config.site).origin).href}`,
    ...
    `Source: ${new URL(withBasePath(sourceUrl ?? markdownUrl), new URL(config.site).origin).href}`,
    "",
  ].join("\n");

...
export function initSearch(config: SearchConfig): SearchInstance {
  ...
  async function ensureInitialized(): Promise<boolean> {
    ...
    try {
      ...
      emptyState.textContent = "搜索在生产构建后可用 / Search requires a production build.";
      return false;
    }
  }
  ...
  async function runSearch(query: string): Promise<void> {
    ...
    emptyState.textContent = "正在搜索 / Searching…";
    clearResults();
    ...
    try {
      ...
      if (results.length === 0) {
        emptyState.style.display = "";
        emptyState.textContent = "没有找到结果 / No results found.";
        return;
      }
      ...
      clearResults();
      emptyState.style.display = "";
      emptyState.textContent = "搜索暂时不可用 / Search is temporarily unavailable.";
    }
  }
  ...
  function handleInput(): void {
    ...
    debounceTimer = setTimeout(() => {
      ...
      if (!query) {
        ...
        clearResults();
        emptyState.style.display = "";
        emptyState.textContent = "输入关键词开始搜索 / Type to search…";
        return;
      }
